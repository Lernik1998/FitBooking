...
<div class="p-6 mx-auto max-w-3xl bg-white rounded-md shadow-md">
    ...
    <form method="POST" action="{{ $action }}" id="form-base" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name', isset($plan) ? $plan->name : '') }}"
        ...
        <div>
            ...
            <input type="number" id="price" name="price" value="{{ old('price', isset($plan) ? $plan->price : '') }}"
                class="block px-4 py-2 w-full text-gray-900 rounded-md border border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        ...
        <div>
            ...
            <select name="duration" id="duration"
                class="block px-4 py-2 w-full text-gray-900 rounded-md border border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <option value="1" {{ (isset($plan) && $plan->duration == '1') ? 'selected' : '' }}>
                    1h</option>
                <option value="4.5" {{ (isset($plan) && $plan->duration == '4.5') ? 'selected' : '' }}>
                    45 minutos
                </option>
            </select>
        </div>
        ...
        <div>
            ...
            <textarea id="description" name="description"
                class="block px-4 py-2 w-full text-gray-900 rounded-md border border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                rows="4">{{ old('description', isset($plan) ? $plan->description : '') }}</textarea>
        </div>


        <!-- Campo de ACTIVO -->
        <div>
            <label for="is_active" class="block mb-2 text-sm font-medium text-gray-700">Activo</label>
            <select name="is_active" id="is_active"
                class="block px-4 py-2 w-full text-gray-900 rounded-md border border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <option value="1" {{ (isset($plan) && $plan->is_active == '1') ? 'selected' : '' }}>
                    Si</option>
                <option value="0" {{ (isset($plan) && $plan->is_active == '0') ? 'selected' : '' }}>
                    No
                </option>
            </select>
        </div>
